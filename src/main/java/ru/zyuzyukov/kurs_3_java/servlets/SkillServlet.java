...
public class SkillServlet extends HttpServlet {
    private final String index = "WEB-INF/views/skill.jsp";
    private BaseService<SkillDto, Skill> service;

    @Override
    public void init(ServletConfig config) throws ServletException {
        super.init(config);
        ApplicationContext context = (ApplicationContext) config.getServletContext().getAttribute("appContext");
        service = context.getSkillService();
    }

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        List<SkillDto> skills = service.findAll();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch (action) {
            case "add":
                service.save(new SkillDto(UUID.randomUUID(), name));
                break;
            case "update":
                UUID id = UUID.fromString(req.getParameter("id"));
                service.update(new SkillDto(id, name));
                break;
            case "delete":
